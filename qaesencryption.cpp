...
QByteArray QAESEncryption::expandKey(const QByteArray& key) {
...
#endif
    {
        ...
        for (i = m_nk; i < m_nb * (m_nr + 1); i++) {
            ...
            roundKey.insert(
                i * 4 + 0, (quint8) roundKey.at((i - m_nk) * 4 + 0) ^ tempa[0]);
            roundKey.insert(
                i * 4 + 1, (quint8) roundKey.at((i - m_nk) * 4 + 1) ^ tempa[1]);
            roundKey.insert(
                i * 4 + 2, (quint8) roundKey.at((i - m_nk) * 4 + 2) ^ tempa[2]);
            roundKey.insert(
                i * 4 + 3, (quint8) roundKey.at((i - m_nk) * 4 + 3) ^ tempa[3]);
        }
        return roundKey;
    }
}
...
QByteArray QAESEncryption::encode(const QByteArray& rawText,
    ...
    case ECB: {
        ...
        if (m_aesNIAvailable) {
            ...
            AES_ECB_encrypt(in, out, alignedText.size(), expKey, m_nr);
    ...
    case CBC: {
        ...
        if (m_aesNIAvailable) {
            ...
            char expKey[expandedKey.size()];
            memcpy(expKey, expandedKey.data(), expandedKey.size());
            AES_CBC_encrypt(in, (unsigned char*) out, ivec, alignedText.size(),
                            expKey, m_nr);
        ...
        for (int i = 0; i < alignedText.size(); i += m_blocklen) {
            alignedText.replace(
                i, m_blocklen, byteXor(alignedText.mid(i, m_blocklen), ivTemp));
            ret.append(cipher(expandedKey, alignedText.mid(i, m_blocklen)));
    ...
    case CFB: {
        ...
        for (int i = 0; i < rawText.size(); i += m_blocklen) {
            if (i + m_blocklen < rawText.size()) {
                ret.append(
                    byteXor(rawText.mid(i + m_blocklen, m_blocklen),
